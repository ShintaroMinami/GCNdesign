...
from gcndesign.training2021 import train, valid
from gcndesign.models import GCNdesign2021, weights_init
...
if args.checkpoint_in != None:
    ...
    ## Model Setup ##
    model = GCNdesign2021(hypara).to(source.device)
...
for iepoch in range(epoch_init, hypara.nepoch):
    ...
    file.flush()
    # output params
    torch.save(model.to('cpu').state_dict(), "{}-{:03d}.pkl".format(source.param_prefix, iepoch))
    torch.save({
        'epoch': iepoch,
